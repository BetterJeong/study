...
public class AddrBean { 
	...
	public boolean updateDB(AddrBook addrbook) {
		...
		String sql ="update addrbook set ab_name=?, ab_email=?, ab_birth=?, ab_tel=?, ab_comdept=?, ab_address=?, ab_memo=? where ab_id=?";		
		...
		try {
			...
			pstmt.setString(6, addrbook.getAb_address());
	...
	public boolean insertDB(AddrBook addrbook) {
		...
		String sql ="insert into addrbook(ab_name,ab_email,ab_birth,ab_tel,ab_comdept,ab_address,ab_memo) values(?,?,?,?,?,?,?)";
	...
	public AddrBook getDB(int gb_id) {
		...
		try {
			...
			addrbook.setAb_address(rs.getString("ab_address"));
